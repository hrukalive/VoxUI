...
fn load_conv(loader: &GgufModelLoader, prefix: &str, groups: usize) -> Result<Conv1dParams> {
...
fn causal_conv1d(x: &Tensor, params: &Conv1dParams, dilation: usize) -> Result<Tensor> {
    ...
    let x = if padding > 0 {
        x.pad_with_zeros(2, padding, 0)?
    } else {
        x.clone()
    };
    let out = x.conv1d(&params.weight, 0, 1, dilation, params.groups)?;
...
fn causal_conv_transpose1d(x: &Tensor, params: &Conv1dParams, stride: usize) -> Result<Tensor> {
    let kernel_size = params.weight.dim(2)?;
    let padding = kernel_size - stride;
    // candle conv_transpose1d: (input, weight, padding, output_padding, stride, dilation, groups)
    // We use padding = kernel_size - stride to get causal behavior, no output_padding.
    let out = x.conv_transpose1d(&params.weight, padding, 0, stride, 1, params.groups)?;
    // Trim right to exactly input_len * stride
...
impl ResidualUnit {
    ...
    dim: usize,
        ...
        Ok(Self { alpha1, dw_conv, alpha2, pw_conv, dilation, dim })
    }
...
impl DecoderBlock {
    fn load(
        ...
        let prefix = format!("audiovae.decoder.model.{model_idx}");

        // SR conditioning - optional (V2 only)
        let (scale, bias) = if let Some(sr_idx) = sr_idx {
            let sr_prefix = format!("audiovae.decoder.sr_cond_model.{model_idx}");
...
impl AudioVAE {
    ...
    fn count_decoder_blocks(loader: &GgufModelLoader) -> usize {
        ...
        for name in &names {
            if let Some(rest) = name.strip_prefix("audiovae.decoder.model.") {
    ...
    fn infer_decoder_rates(loader: &GgufModelLoader, num_blocks: usize) -> Result<Vec<usize>> {
        ...
        for i in 0..num_blocks {
            ...
            let name = format!("audiovae.decoder.model.{model_idx}.block.1.weight_v");
    ...
    pub fn load(loader: &GgufModelLoader, config: AudioVAEConfig) -> Result<Self> {
        ...
        let dw_conv = load_conv(loader, "audiovae.decoder.model.0", config.latent_dim)?;
        // model.1: pointwise conv latent_dim -> decoder_dim
        let pw_conv = load_conv(loader, "audiovae.decoder.model.1", 1)?;
        ...
        let final_alpha = loader.load_tensor(&format!("audiovae.decoder.model.{final_snake_idx}.alpha"))?;
        let final_conv = load_conv(loader, &format!("audiovae.decoder.model.{final_conv_idx}"), 1)?;

        Ok(Self { dw_conv, pw_conv, blocks, final_alpha, final_conv })
    }
